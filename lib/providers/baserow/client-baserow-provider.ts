...
import { BaserowRow, BaserowPaginatedResponse, BaserowFieldMetadata } from "./types"
...
export class ClientBaserowProvider implements IDataProvider {
   ...
   */
  private async getTableFields(tableId: string): Promise<BaserowFieldMetadata[]> {
  ...
  private async extractStatusesFromSingleSelect(): Promise<TaskStatus[]> {
    try {
      ...
      const fields = await this.getTableFields(this.config.tasksTableId)
